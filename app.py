"""
Melanoma Skin Cancer Detection – Gradio app
...
_TMP_IMG    = '/tmp/melanoma_upload.jpg'

# ── Load model ─────────────────────────────────────────────────────────────────
def _load_model():
    """Load from local file if present, otherwise download from HF Hub."""
    local_path = 'best_melanoma_model.keras'
    if os.path.exists(local_path):
        print(f"Loading model from local file: {local_path}")
        return tf.keras.models.load_model(local_path)

    # Download from HF Hub model repo (set HF_MODEL_REPO env var or edit below)
...
def predict_image(pil_image, threshold=0.5):
    """Return (predicted_class, confidence) for a PIL image."""
    ...
    if prob_malignant >= threshold:
        return CLASS_NAMES[1], prob_malignant          # malignant
    return CLASS_NAMES[0], 1.0 - prob_malignant        # benign


def _risk_level(pred_cls: str, conf: float):
...
with gr.Blocks(theme=_THEME, title='Melanoma Detection') as demo:
    ...
    with gr.Row(equal_height=True):
        with gr.Column(scale=1, min_width=300):
            ...
            with gr.Row():
                ...
                clear_btn   = gr.ClearButton(components=[img_input], value='🗑  Clear', size='lg')
    ...
    analyse_btn.click(fn=melanoma_inference, inputs=img_input, outputs=[label_out, report_out], api_name='predict')
    img_input.upload(fn=melanoma_inference, inputs=img_input, outputs=[label_out, report_out])
